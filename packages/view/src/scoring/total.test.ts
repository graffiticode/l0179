...
describe("score", () => {
  ...
  test("scores nothing answered as zero out of the total", () => {
    expect(score({ validation: oneSheetValidation("4", 2) })).toEqual({ score: 0, max: 2 });
  });
  ...
  test("sums the points of the right answers in the response map", () => {
    ...
    expect(score(data)).toEqual({ score: 2, max: 2 });
    expect(score({ ...data, cells: { A1: { text: "5", val: "5", type: "number" } } }))
      .toEqual({ score: 0, max: 2 });
  });
  ...
  test("with several sheets each sheet's answer counts against its own key", () => {
    ...
    expect(score({ validation, cells })).toEqual({ score: 5, max: 5 });
    expect(score({ validation, cells: { "s2!A1": cells["s2!A1"] } })).toEqual({ score: 3, max: 5 });
  });
});
