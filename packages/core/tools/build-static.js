// Emits L0179's public static assets into dist/static/. As a child of L0000, L0179 merges
// inherited content from its parent:
//   - lexicon.json: the merged lexicon (base + L0179) — already merged in src/lexicon.ts.
//   - instructions.md: parent (L0000) instructions concatenated with L0179's.
// The rest (spec.html, language-info.json, scope.json, schema.json, template.gc,
// usage-guide.md) are L0179's own. Ported from L0176's build-static.js.
...
// 1. lexicon — merged (base + L0179) as plain JSON. Unlike L0176 there are no deprecated
//    aliases to filter: L0179 is new, so every word it publishes is a current spelling.
...
// 3. instructions.md — parent (L0000) instructions + L0179's.
...
// 4. Copy L0179's own verbatim spec assets.
...
//    Extracted rather than duplicated so the agent-facing guide and the catalog entry cannot
//    drift apart.
